...
impl Backend for EnvironmentBackend {
    ...
    fn run(&self, config: &Config, task: &str, args: &[String]) -> Result<()> {
        let project_environment = ProjectEnvironment::load(&self.root)?;
        self.run_with_environment(config, task, args, &project_environment)
    }
    ...
    fn run_with_environment(
        ...
        project_environment.apply_to_command(&mut command);
    ...
    fn shell(&self, config: &Config) -> Result<()> {
        let project_environment = ProjectEnvironment::load(&self.root)?;
        self.shell_with_environment(config, &project_environment)
    }
    ...
    fn shell_with_environment(
        ...
        #[cfg(not(windows))]
        {
            unix_shell(self, config, project_environment)
        }
    }
