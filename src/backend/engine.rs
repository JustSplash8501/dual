...
if (interactive()) {{
    dual_project <- normalizePath(
        Sys.getenv("DUAL_PROJECT_ROOT", unset = getwd()),
        winslash = "/",
        mustWork = FALSE
    )
    dual_home <- normalizePath(path.expand("~"), winslash = "/", mustWork = FALSE)
    dual_display <- if (startsWith(dual_project, paste0(dual_home, "/"))) {{
        paste0("~/", substring(dual_project, nchar(dual_home) + 2L))
    }} else {{
        dual_project
    }}
    packageStartupMessage(
        sprintf("- Project '%s' loaded. [dual {}]", dual_display)
    )
    rm(dual_project, dual_home, dual_display)
}}
"#,
